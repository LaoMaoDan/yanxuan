    ...
    <div class="header">
      <div class="search">
        <div class="search_input">
          ...
          <div v-show="show" class="searchHisetory">
            <h4>历史记录</h4>
            <div class="zan" v-if="!historyArr.length">暂无历史记录</div>
            <div v-else v-for="(item, index) in historyArr" :key="index">
              {{ item }}
            </div>
          </div>
        </div>
...
export default {
  ...
  activated() {
    ...
    if (this.$store.state.routerFrom || this.$store.state.flag) {
      this.getList();
    }
  },
  beforeRouteLeave(to, from, next) {
    if (from.path == "/goodsList" && to.path == "/cate") {
      console.log(to.meta,32156);
      // this.$store.state.routerFrom = false;
  ...
  methods: {
    ...
    goDetails(val) {
      localStorage.setItem("goodListAttributes", JSON.stringify(val));
