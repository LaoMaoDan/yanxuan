<template>
  <div class="box">
    <el-message v-if="show1" :type="type" :title="title"></el-message>
    ...
    <div class="submit" v-show="list.length > 0">
      <div class="checked">
        <div class="check">
          <div class="checkbox">
            <input type="checkbox" @click="eidtCheckedAll" v-model="all" />
          </div>
        </div>
        全选
      </div>
...
export default {
  ...
  methods: {
    ...
    ...mapMutations("cart", {incrementBy: "setCheckedAll"}, ["setChecked"]),

    eidtCheckedAll() {
      this.$store.commit("setCheckedAll", this.all);
    },
    open(val) {
      this.delGood = val;
      this.show = true;
