<template>
  <div class="box">
    <el-message v-if="show" :title="title"></el-message>
...
export default {
  ...
  methods: {
    ...
    async login() {
      // 校验是否通过
      console.log(this.other, this.check);
      let other = this.other.some((item) => {
        return item;
      });
      if (this.other.length > 2) console.log(other);
      if (this.other && this.check) {
