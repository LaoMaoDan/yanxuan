<template>
  <div class="box">
    ...
    <div class="contentBox" ref="contentBox" @touchstart="start($event)">
      ...
      <div class="content" ref="content" v-pull="obj">
        <div
          class="fox"
...
export default {
  ...
  beforeRouteLeave(to, from, next) {
    if (from.path == "/goodsList" && to.path == "/cate") {
      this.$destroy();
    }
    next();
  },
  ...
  methods: {
    start(e) {
      let contentBox = null;
      let content = null;
      let startX = e.changedTouches[0].clientY;
      let moveX = 0;
      this.$nextTick((_) => {
      ...
      const move = (e) => {
        moveX = e.changedTouches[0].clientY;
        let Y = moveX - startX;
        if (Y < content.scorllHeight - content.clientHeight) {
          Y = content.scorllHeight - content.clientHeight;
        }
        console.log(content.scrollHeight, contentBox.clientHeight);
