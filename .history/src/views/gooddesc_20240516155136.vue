  ...
  <div class="box">
    ...
    <footer>
      ...
      <router-link tag="div" to="/cart" class="fox">
        <div class="num" ref="num" v-if="list.length">{{ list.length }}</div>
...
export default {
  ...
  methods: {
    ...
    async inCart() {
      ...
      if (res.list.cartCount > 0) {
        ...
        let numright = -5.0025;
        let right =
          -document.documentElement.offsetWidth + num.getBoundingClientRect().x;
        let top = right;

        num.style.right = right / 75 + "rem";
        num.style.top = top / 75 + "rem";
        let timer = setInterval(() => {
          if (right >= -numright && top >= numright) {
            num.style.right = -numright / 75 + "rem";
            num.style.top = numright / 75 + "rem";
            clearInterval(timer);
          }
          right += 2;
          num.style.right = right / 75 + "rem";
  ...
  async created() {
    ...
    this.$store.state.routerFrom = !flag;
