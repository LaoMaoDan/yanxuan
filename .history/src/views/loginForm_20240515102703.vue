...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      if (this.check && other) {
        // this.throttle();
        this.throttle(this.postLogin, 1000)();
      } else {
        this.$message.show({ title: "请输入账号密码", type: "error" });
      }
    },
    async postLogin() {
      await postLoginApi(this.userInfo)
        .then((res) => {
          ...
          this.$message.show({ title: "登录成功", type: "success" });
          this.$router.push("/index");
        })
        .catch((res) => {
          this.$message.show({ title: res.msg, type: "error" });
        });
    },
    // 防抖
    debounce(fn,delay){
...
原文链接：https://blog.csdn.net/weixin_48594833/article/details/123460177
    },
    register() {
